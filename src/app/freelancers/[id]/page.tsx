    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-4">This freelancer profile doesn&apos;t exist.</p>
  ...
  return (
    <div className="min-h-screen py-8 bg-grid relative">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        <Link href="/freelancers" className="inline-flex items-center gap-2 text-gray-600 hover:text-gray-900 mb-6 transition-colors">
        ...
        <div className="glass rounded-2xl p-8 mb-6">
          <div className="flex flex-col sm:flex-row items-start gap-6">
            <div className={`w-24 h-24 rounded-2xl bg-gradient-to-br ${gradient} flex items-center justify-center text-gray-900 text-3xl font-bold shrink-0`}>
              {initials}
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex flex-wrap items-center gap-3 mb-2">
                ...
                {profile.isDataContributor && (
                  <span className="badge badge-primary"><Database className="w-3 h-3" /> Data Contributor</span>
                )}
                {cp?.isVerified && (
              ...
              <p className="text-lg text-gray-600 mb-4">{profile.tagline || "AI Professional"}</p>
              ...
              <div className="flex flex-wrap gap-2">
                {profile.skills.map((skill) => (
                  <span
                    key={skill.id}
                    className={`badge ${
                      skill.category === "Data Collection" ? "badge-primary" :
                      skill.category === "Specialties" ? "badge-purple" : "badge-primary"
                    }`}
                  >
                    {skill.name}
                  </span>
              ...
              <div className="flex flex-wrap gap-2">
                {profile.languages.length > 0 ? (
                  profile.languages.map((lang) => (
                    <span key={lang} className="px-3 py-1 rounded-full bg-gray-100 text-sm text-gray-700 border border-gray-200">
                      {lang}
                    </span>
                  ))
                ) : (
              ...
              <div className="glass rounded-2xl p-6 border-teal-500/20">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-semibold text-gray-900">{cp.tasksCompleted}</span>
                  </div>
                  <div className="flex justify-between items-center">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-semibold text-gray-900">{cp.datasetsContributed}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-gray-600">Languages</span>
                    <span className="font-semibold text-gray-900">{cp.languagesSpoken.length}</span>
                  </div>
                </div>
              </div>
            )}
