  ...
  return (
    <Link href={`/freelancers/${freelancer.id}`} className="glass-card rounded-2xl p-6 block group">
      <div className="flex items-start gap-4 mb-4">
        <div className={`w-14 h-14 rounded-xl bg-gradient-to-br ${gradient} flex items-center justify-center text-gray-900 font-bold text-lg shrink-0`}>
          {initials}
        </div>
        <div className="min-w-0">
          ...
          <p className="text-sm text-gray-600 truncate">{freelancer.tagline || "AI Professional"}</p>
        </div>
      </div>

      <div className="flex flex-wrap gap-1.5 mb-4">
        {freelancer.skills.slice(0, 4).map((skill) => (
          <span key={skill.id} className="px-2 py-0.5 rounded-md bg-gray-100 text-xs text-gray-600 border border-gray-200">
            {skill.name}
          </span>
        ))}
        ...
        <div className="flex items-center gap-4">
          {freelancer.hourlyRate && (
            <span className="text-sm font-semibold text-gray-900">
              ${freelancer.hourlyRate}<span className="text-gray-500 font-normal">/hr</span>
            </span>
          )}
  ...
  return (
    <div className="min-h-screen py-8 bg-grid relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Browse AI Talent</h1>
          <p className="text-gray-600">Find the perfect AI professional for your project</p>
        </div>
