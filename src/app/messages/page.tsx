          ...
          <div className="grid grid-cols-12 h-full">
            ...
            <div className="col-span-4 border-r border-gray-200 flex flex-col">
              ...
              <div className="flex-1 overflow-y-auto workspace-scroll">
                {THREADS.map((t, i) => (
                  <button key={t.id} onClick={() => setActiveThread(t)} className={`w-full p-4 text-left transition-all flex items-center gap-3 border-b border-gray-200 ${activeThread.id === t.id ? "bg-blue-500/5" : "hover:bg-white/[0.02]"}`}>
                    <div className={`w-10 h-10 rounded-full bg-gradient-to-br ${gradients[i % gradients.length]} flex items-center justify-center text-gray-900 font-bold text-sm shrink-0`}>{t.avatar}</div>
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex items-center justify-between">
                        <p className="text-xs text-gray-500 truncate">{t.lastMsg}</p>
                        {t.unread > 0 && <span className="w-5 h-5 rounded-full bg-blue-500 text-gray-900 text-xs flex items-center justify-center shrink-0">{t.unread}</span>}
                      </div>
                    </div>
                  </button>
                ))}
            ...
            <div className="col-span-8 flex flex-col">
              ...
              <div className="p-4 border-b border-gray-200 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-gray-900 font-bold">{activeThread.avatar}</div>
                  <div><p className="text-sm font-semibold text-gray-900">{activeThread.name}</p><p className="text-xs text-gray-500">{activeThread.role}</p></div>
                </div>
                <div className="flex gap-2">
                  <button className="p-2 rounded-lg hover:bg-gray-100 text-gray-600"><Phone className="w-4 h-4" /></button>
                  <button className="p-2 rounded-lg hover:bg-gray-100 text-gray-600"><Video className="w-4 h-4" /></button>
                ...
                {messages.map(m => (
                  <div key={m.id} className={`flex ${m.sender === "me" ? "justify-end" : "justify-start"}`}>
                    <div className={`max-w-[70%] px-4 py-3 rounded-2xl text-sm ${
                      m.sender === "me" ? "bg-blue-500/20 text-blue-100 rounded-br-md" : "bg-gray-100 text-gray-700 rounded-bl-md"
                    }`}>
                      <p>{m.text}</p>
              ...
              <div className="p-4 border-t border-gray-200">
                <div className="flex items-center gap-3">
                  <button className="p-2 rounded-lg hover:bg-gray-100 text-gray-600"><Paperclip className="w-5 h-5" /></button>
