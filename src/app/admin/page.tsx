    ...
    <div className="min-h-screen py-8 bg-grid">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center gap-3 mb-8">
          ...
          <div><h1 className="text-3xl font-bold text-white">Admin Panel</h1><p className="text-slate-400 text-sm">Manage users, jobs, and platform</p></div>
        </div>
        ...
        <div className="flex gap-1 p-1 rounded-xl bg-white/5 mb-6 w-fit">
          {(["users", "jobs", "campaigns"] as const).map(t => (
            <button key={t} onClick={() => setTab(t)} className={`px-4 py-2 rounded-lg text-sm font-medium capitalize transition-all ${tab === t ? "bg-blue-500/20 text-blue-400" : "text-slate-500 hover:text-slate-300"}`}>{t}</button>
          ))}
        </div>
          ...
          <div className="glass rounded-2xl overflow-hidden">
            ...
            <table className="w-full">
              <thead><tr className="border-b border-white/5">
                ...
                <th className="text-left p-4 text-xs font-medium text-slate-500 uppercase">Activity</th>
              ...
              <tbody>
                {filteredUsers.map(u => (
                  <tr key={u.id} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors">
                    ...
                    <td className="p-4"><span className={`badge text-xs ${u.role === "CLIENT" ? "badge-blue" : "badge-purple"}`}>{u.role}</span></td>
                    <td className="p-4"><span className={`badge text-xs ${u.status === "active" ? "badge-green" : "badge-red"}`}>{u.status}</span></td>
                    <td className="p-4"><p className="text-sm text-slate-400">{u.jobs} jobs</p></td>
                    <td className="p-4"><p className="text-sm text-slate-500">{new Date(u.joined).toLocaleDateString()}</p></td>
                    <td className="p-4">
        ...
        {tab === "jobs" && (
          <div className="glass rounded-2xl p-8 text-center">
            ...
            <p className="text-slate-400">Job management view — all active and completed jobs</p>
          </div>
        )}
        ...
        {tab === "campaigns" && (
          <div className="glass rounded-2xl p-8 text-center">
            ...
            <p className="text-slate-400">Data campaign management — track all collection campaigns</p>
          </div>
        )}
      </div>
    </div>
